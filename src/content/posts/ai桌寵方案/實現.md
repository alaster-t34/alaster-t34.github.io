...
├── **03.物理表现层** (physic_engine)
│   ├── live2d/
|   │   ├── neural_rigging/                   # 新增AI骨骼驱动系统
|   │   │   ├── emotion_deform.py            # 情绪驱动骨骼形变
|   │   │   ├── gait_learner.py              # 深度学习步态生成器
|   │   │   └── micrexpression_controller/   # 微表情控制中枢
|   │   │       ├── blink_system.py          # 自主眨眼控制（时序噪声+情感驱动）
|   │   │       └── mouth_sync/              # 高级口型同步
|   │   │           ├── viseme_mapping.json  # 音素-口型对照表
|   │   │           └── coarticulation.py    # 发音协同修正
|   ├── physics/
|   │   ├── autonomous_controller/            # 新增自主行为控制器
|   │   │   ├── pathfinding.py               # A*与RVO结合路径规划
|   │   │   ├── motion_planner.py            # 动作序列生成器
|   │   │   └── motion_transition/           # 动作过渡处理
|   │   │       ├── curve_generator.py       # 贝塞尔路径平滑
|   │   │       └── collision_avoidance.py   # 实时碰撞规避
|   │   └── behavior_orchestrator/           # 新增行为编排中枢
|   │       ├── priority_stack.py            # 行为优先级管理
|   │       └── intent_translator.py         # 意图→动作转换器
|   └── integration/
|       └── cognitive_bridge.py             # 认知-动画桥接模块
│
├── **04.人机交互层** (interaction)
│   ├── voice/                      # 语音交互系统
│   │   ├── synthesis/
